...
type Selector struct {
	sel   interface{}
	exp   int
	by    func(context.Context, FrameHandler, *Node) ([]NodeID, error)
	wait  func(context.Context, FrameHandler, *Node, ...NodeID) ([]*Node, error)
	after func(context.Context, FrameHandler, ...*Node) error
}
...
func (s *Selector) Do(ctxt context.Context, h FrameHandler) error {
	// TODO: fix this
...
// funcs.
func (s *Selector) run(ctxt context.Context, h FrameHandler) chan error {
	ch := make(chan error)
...
// met, will execute f.
func QueryAfter(sel interface{}, f func(context.Context, FrameHandler, ...*Node) error, opts ...QueryOption) Action {
...
func ByFunc(f func(context.Context, FrameHandler, *Node) ([]NodeID, error)) QueryOption {
...
func ByQuery(s *Selector) {
	ByFunc(func(ctxt context.Context, h FrameHandler, n *Node) ([]NodeID, error) {
...
func ByNodeID(s *Selector) {
	ids, ok := s.sel.([]NodeID)
	if !ok {
		panic("ByNodeID can only work on []NodeID")
	}
...
func (s *Selector) waitReady(check func(context.Context, FrameHandler, *Node) error) func(context.Context, FrameHandler, *Node, ...NodeID) ([]*Node, error) {
	return func(ctxt context.Context, h FrameHandler, n *Node, ids ...NodeID) ([]*Node, error) {
...
func WaitFunc(wait func(context.Context, FrameHandler, *Node, ...NodeID) ([]*Node, error)) QueryOption {
	return func(s *Selector) {
		s.wait = wait
	}
}
...
func ElementVisible(s *Selector) {
	WaitFunc(s.waitReady(func(ctxt context.Context, h FrameHandler, n *Node) error {
		var err error
...
// has succeeded.
func After(f func(context.Context, FrameHandler, ...*Node) error) QueryOption {
	return func(s *Selector) {
		s.after = f
	}
}
